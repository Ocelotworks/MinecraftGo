...
func NBTStructScan(obj interface{}, compound *nbt.Compound) {
	...
	for name, tag := range compound.Data {
		...
		if tag.GetType() == 9 {
			...
			field.Set(ReflectValueFromNBT(tag, fieldType.Type))
		}
	}
}

func NBTStructToCompound(obj interface{}) nbt.Compound {
	v := reflect.ValueOf(obj).Elem()
	t := reflect.TypeOf(obj).Elem()

	compoundMap := make(map[string]nbt.NBTValue)
	...
	for fieldIndex := 0; fieldIndex < t.NumField(); fieldIndex++ {
		...
		if field.Type.Kind() == reflect.Slice {
			outputValues := make([]nbt.NBTValue, fieldValue.Len())
			for i := range outputValues {
				outputValues[i] = NBTValueFromReflect(fieldValue.Index(i))
			}
			nbtList := nbt.NewList(outputValues)
	...
	return nbt.NewCompound(compoundMap)
}

func NBTValueFromReflect(v reflect.Value) nbt.NBTValue {
	id := nbt.IDFromType(v.Type().Kind().String())

	nbtValue := nbt.NewValue(id)
...
func ReflectValueFromNBT(v nbt.NBTValue, fieldType reflect.Type) reflect.Value {
	if v.GetType() != 10 {
		return reflect.ValueOf(v.GetValue())
	}

	objectValue := reflect.New(fieldType)
	NBTStructScan(objectValue.Interface(), v.(*nbt.Compound))
